...
> python mgui.py --simtype=JTB --paper=0 --figure=3
...
class CellPanel(object):#(wx.Panel):
    def __init__(self,app,cpname='CP_JTB',xsz=600,ysz=800,makepanel=True):
        self.app= app
    ...
    def angled_linetext(self, ctx, angle, li,lj,lk,ll, textlines, spacing=2, lineoffset=1.0, dx=0, dy=0, dr=0, dc=0, debug=[]):
        ...
        for i,tl in enumerate(textlines):
            ...
            if abs(angle) < 90:
                ...
                ctx.rotate(math.pi-radians) # ROTATE is positive clockwise
            ctx.move_to(0,0)
...
class CellPanel_Oocyte(CellPanel):
    ...
    def Render(self, dc): # CellPanel_Oocyte
        ...
        self.angled_linetext( ctx, 0, li,lj,lk,ll, textlines)#, debug=['initcross'] )
...
class ModelApp(object):
    ...
    def __init__(self, myargs):
        ...
        self.hsiz_save_cur_params.Add( self.b_save_cur_params, 0, wx.EXPAND )
        self.hsiz_save_cur_params.AddSpacer(spacer_size)
        self.hsiz_save_cur_params.Add( self.t_save_cur_params, 0, wx.EXPAND )

        self.hsiz_run_cur_sim = wx.BoxSizer(wx.HORIZONTAL)
        self.hsiz_run_cur_sim.AddSpacer(v_spacer_size)
        self.hsiz_run_cur_sim.Add( self.b_run_cur_sim, 0, wx.EXPAND )
        self.hsiz_run_cur_sim.AddSpacer(spacer_size)
        self.hsiz_run_cur_sim.Add( self.t_run_cur_sim, 0, wx.EXPAND )
    ...
    def OnSelectSimType(self,e,simtype=None,paper=None,fig=None):
        ...
        print('avail sims=', self.sims('n','l'))
...
class SimResults():
    ...
    def make_output(self,outfile,indexvarname,indexvar,datadict): # SimResults
        ...
        thisdf= pd.DataFrame( datadict, index=indexvar, )
        print('thisdf=',thisdf)
